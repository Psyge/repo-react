...
export default function GlobeView({ premium = false, onFallback, onUpgrade }) { //
  ...
  /* --- ORGAANINEN VÄRISIIRTYMÄ ---
     Siirretään useCallback-funktioon, jotta se on vakaa riippuvuus
     ja useMemo:n optimointi toimii oikein.
  */
  ...
  // TÄSSÄ KORJAUS:
  // 1. Käytetään dynaamisena riippuvuutena koko 'auroraPoints'-taulukkoa
  //    noudattaaksemme 'react-hooks/exhaustive-deps'-sääntöä.
  // 2. Lisätään 'getAuroraColor' riippuvuuslistaan. Koska se on siirretty
  //    useCallback-funktioon, se on vakaa eikä aiheuta turhia uudelleenlaskentoja
  //    silloin kun auroraPoints-taulukko ei muutu.
  const auroraColors = useMemo(() => { //
    return auroraPoints.map(p => getAuroraColor(p.val)); //
  }, [auroraPoints, getAuroraColor]); // Korjattu riippuvuuslista
